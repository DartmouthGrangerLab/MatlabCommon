%no, matlab, this should NOT be this hard.
%http://www.mathworks.com/matlabcentral/fileexchange/23573-csvimport/content/csvimport.m
...
%                                                if conversion of any data element in the column fails. Default value is false.
%                                uniformOutput - true / false value indicating whether output can be
%                                                returned without encapsulation in a cell array.
%                                                This parameter is ignored if the columns / table cannot be converted into a matrix.
%                                noHeader      - true / false value indicating whether the CSV
%                                                file's first line contains column headings. Default value is false.
%                                ignoreWSpace  - true / false value indicating whether to ignore
%                                                leading and trailing whitespace in the column
%                                                headers; ignored if noHeader is set to true. Default value is false.
% The parameters must be specified in the form of param-value pairs, parameter names are not case-sensitive and partial matching is supported.
...
function varargout = CSVImport (fileName, varargin)
    if (nargin == 0) || isempty(fileName)
        ...
        if ~ischar(fileName)
            error('csvimport:FileNameError', 'The first argument to %s must be a valid .csv file', mfilename );
        end
    end
    ...
    validParams = {'delimiter','columns','outputAsChar','uniformOutput','noHeader','ignoreWSpace'};

    %Parse input arguments
    if nargin > 1
        if mod( numel( varargin ), 2 ) ~= 0
            error('csvimport:InvalidInput', ['All input parameters after the fileName must be in the form of param-value pairs']);
        end
        ...
        for ii = 1:numel(params)
            ...
            if numel(result) > 1
                ...
                if ~isempty( exresult )
                    ...
                    error('csvimport:InvalidInput', 'Cannot find unambiguous match for parameter ''%s''', varargin{ii*2-1} );
                end
            end
    ...
    if isempty(p.delimiter) || ~ischar(p.delimiter)
        error('csvimport:InvalidParamType', 'The ''delimiter'' parameter must be a non-empty character array');
    end
    if isempty(p.noHeader) || ~islogical(p.noHeader) || ~isscalar(p.noHeader)
        error('csvimport:InvalidParamType', 'The ''noHeader'' parameter must be a non-empty logical scalar');
    end
    if ~p.noHeader
        if ~isempty(p.columns)
            if ~ischar(p.columns) && ~iscellstr(p.columns)
                error('csvimport:InvalidParamType', 'The ''columns'' parameter must be a character array or a cell array of strings for CSV files containing column headers on the first line');
            end
            if p.ignoreWSpace
        ...
        if ~isempty(p.columns) && ~isnumeric(p.columns)
            error('csvimport:InvalidParamType', 'The ''columns'' parameter must be a numeric array for CSV files containing column headers on the first line');
        end
    end
    if isempty(p.outputAsChar) || ~islogical(p.outputAsChar) || ~isscalar(p.outputAsChar)
        error('csvimport:InvalidParamType', 'The ''outputAsChar'' parameter must be a non-empty logical scalar');
    end
    if isempty(p.uniformOutput) || ~islogical(p.uniformOutput) || ~isscalar(p.uniformOutput)
        error('csvimport:InvalidParamType', 'The ''uniformOutput'' parameter must be a non-empty logical scalar');
    end

    %Open file
    [fid,msg] = fopen(fileName, 'rt');
    if fid == -1
        error('csvimport:FileReadError', 'Failed to open ''%s'' for reading.\nError Message: %s', fileName, msg);
    end
    ...
    if colMode
        if (nargout > 1) && (nargout ~= nHeaders)
            error('csvimport:NumOutputs', 'The number of output arguments must be 1 or equal to the number of column names when fetching data for specific columns');
        end
    end
    ...
    if colMode
        if ~p.noHeader
            ...
            for ii = 1:nHeaders
                ...
                if isempty(result)
                    fclose(fid);
                    error('csvimport:UnknownHeader', 'Cannot locate column header ''%s'' in the file ''%s''. Column header names are case sensitive.', p.columns{ii}, fileName);
                elseif numel( result ) > 1
                    exresult = strmatch( p.columns{ii}, rowData, 'exact' );
                    if numel(exresult) == 1
                        result = exresult;
                    else
                        warning('csvimport:MultipleHeaderMatches', 'Column header name ''%s'' matched multiple columns in the file, only the first match (C:%d) will be used.', p.columns{ii}, result(1));
                    end
                end
                colIdx(ii)  = result(1);
            end
            ...
            if max(colIdx) > nCols
                fclose(fid);
                error('csvimport:BadIndex', ['The specified column index ''%d'' exceeds the number of columns (%d) in the file'], max( colIdx ), nCols );
            end
        end
    end
    ...
    for ii = 2:nLines
        ...
        if nDataElems < nCols
            warning( 'csvimport:UnevenColumns', 'Number of data elements on line %d (%d) differs from that on the first line (%d). Data in this line will be padded.', ii, nDataElems, nCols);
            rowData(nDataElems+1:nCols) = {''};
        elseif nDataElems > nCols
            warning('csvimport:UnevenColumns', ['Number of data elements on line %d (%d) differs from that one the first line (%d). Data in this line will be truncated.'], ii, nDataElems, nCols );
    ...
    if ~colMode
        if ~p.outputAsChar
            ...
            for ii = 1:nCols
                ...
                %If any row contains an entry that cannot be converted to a number then return the whole column as a char array
        ...
        if ~p.outputAsChar
            ...
            for ii = 1:nHeaders
                ...
                if ~any(cellfun(@isnan, colData))
                    ...
                    %If any column cannot be converted to a number then we cannot convert the output to an array or matrix i.e. uniform output is not possible
