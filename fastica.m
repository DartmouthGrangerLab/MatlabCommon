...
%
% FASTICA(mixedsig) estimates the independent components from given multidimensional signals.
% Each row of matrix mixedsig is one observed signal.
% FASTICA uses Hyvarinen's fixed-point algorithm,
% see http://www.cis.hut.fi/projects/ica/fastica/.
% Output from the function depends on the number output arguments:
%
% icasig = FASTICA(mixedsig);
% % the rows of icasig contain the estimated independent components.
% [icasig,A,W] = FASTICA(mixedsig);
% % outputs the estimated separating matrix W and the corresponding mixing matrix A
% [A,W] = FASTICA(mixedsig);
% % gives only the estimated mixing matrix A (vectorSize x numOfIC) and the separating matrix W
%
% Some optional arguments induce other output formats, see below.
%
% A graphical user interface for FASTICA can be launched by the command FASTICAG
%
...
%
% 'approach'            (string) The decorrelation approach used.
%                       Can be symmetric ('symm'), i.e. estimate all the
...
%                       DEFAULT = 'defl'
%
% 'numOfIC'             (integer) Number of independent components to be estimated.
%                       DEFAULT = the dimension of data
%======================================================================
...
%                       'skew'             g(u)=u^2
%
% 'finetune'		    (string) Chooses the nonlinearity g used when fine-tuning.
%                       In addition to same values as for 'g', the possible value 'finetune' is:
...
%
% 'epsilon'             (number) stopping criterion
%                       DEFAULT = 0.0001
%
...
%
% 'verbose'             (string) 'on' | 'off'
%                       DEFAULT = 'on'
%
% 'displayMode'         (string) Plot running estimates of independent components: 'signals', 'basis', 'filters' or 'off'.
%                       DEFAULT = 'off'
%
% 'displayInterval'     Number of iterations between plots.
%                       DEFAULT = 1 (plot after every iteration)
%
%======================================================================
% --Controlling reduction of dimension and whitening:
%
% Reduction of dimension is controlled by 'firstEig' and 'lastEig', or alternatively by 'interactivePCA'.
%
% 'firstEig'            (integer) This and 'lastEig' specify the range for
%                       eigenvalues that are retained, 'firstEig' is
%                       the index of largest eigenvalue to be retained.
%                       DEFAULT = 1
%
% 'lastEig'             (integer) This is the index of the last (smallest) eigenvalue to be retained.
%                       DEFaULT = the dimension of data
%
% 'interactivePCA'      (string) 'on' | 'off'
%                       When set 'on', the eigenvalues are shown to the user and the range can be specified interactively.
%                       DEFAULT = 'off'
%                       Can also be set to 'gui'. Then the user can use the same GUI that's in FASTICAG.
%
% If you already know the eigenvalue decomposition of the covariance
% matrix, you can avoid computing it again by giving it with the following options:
...
% algorithm it still must be entered - some values are still calculated from it.
%
% Performing preprocessing only is possible by the option:
%
% 'only'                (string) Compute only PCA i.e. reduction of
%                       dimension ('pca') or only PCA plus whitening ('white').
%                       DEFAULT = 'all': do ICA estimation as well.  
%                       This option changes the output format accordingly. For example:
%
...
%                       eigenvalue (D) matrices containing the selected subspaces.
%======================================================================
% EXAMPLES
%       icasig = FASTICA (mixedsig, 'approach', 'symm', 'g', 'tanh');
%               Do ICA with tanh nonlinearity and in parallel
%               (like maximum likelihood estimation for supergaussian data).
%
...
%               Don't output convergence reports and don't plot independent components.
%
% A graphical user interface for FASTICA can be launched by the command FASTICAG
% @(#)$Id: fastica.m,v 1.14 2005/10/19 13:05:34 jarmo Exp $
% changed by Eli Bowen only to compress comments and standardize some coding conventions
function [Out1,Out2,Out3] = fastica(mixedsig, varargin)
    % check some basic requirements of the data
    assert(nargin() > 0, 'you must supply the mixed data as input argument');
    assert(numel(size(mixedsig)) <= 2, 'input data can not have more than two dimensions');
    assert(~any(isnan(mixedsig(:))), 'input data must not contain NaNs');
    if ~isa(mixedsig, 'double')
        fprintf('Warning: converting input data into double precision.\n');
        ...
        for i = 1:2:(length(varargin)-1)
            ...
            switch lower (varargin{i})
                ...
                case 'verbose'
                    ...
                    if strcmp (verbose, 'off'), b_verbose = 0; end % silence this program also
                case 'firsteig'
    ...
    % Check if the data has been entered the wrong way, but warn only... it may be on purpose
    if Dim > NumOfSampl
        if b_verbose
            fprintf('Warning: ');
    ...
    if only > 1
        ...
        if jumpWhitening == 3
            ...
            [whitesig, whiteningMatrix, dewhiteningMatrix] = whitenv(mixedsig, E, D, verbose);
        end
    end % if only > 1
...
function [A,W] = fpica(X, whiteningMatrix, dewhiteningMatrix, approach, ...
    ...
    guess, sampleSize, displayMode, displayInterval, s_verbose)
    ...
    myyOrig = myy; % when we start fine-tuning we'll set myy = myyK * myy
    myyK = 0.01;
        ...
        case 'guess'
            if size(guess,1) ~= size(whiteningMatrix,2)
                ...
                if size(guess,2) < numOfIC
                    ...
                    guess(:, size(guess, 2) + 1:numOfIC) = rand(vectorSize,numOfIC-size(guess,2))-.5;
    ...
    if approachMode == 1
        ...
        for round = 1 : maxNumIterations + 1
            if round == maxNumIterations + 1
                ...
                if ~isempty(B)
                    B = B * real(inv(B' * B)^(1/2)); % symmetric orthogonalization
                    W = B' * whiteningMatrix;
            ...
            if (1 - minAbsCos < epsilon)
                if finetuningEnabled && notFine
                    ...
                    A = dewhiteningMatrix * B; % calculate the de-whitened vectors
                    break
                end
    ...
    if approachMode == 2
        ...
        while round <= numOfIC
            ...
            while i <= maxNumIterations + gabba
                ...
                % Project the vector into the space orthogonal to the space spanned by the earlier found basis vectors.
                % Note that we can do the projection with matrix B, since the zero entries do not contribute to the projection.
                w = w - B * B' * w;
                w = w / norm(w);
                ...
                % Note that the algorithm has converged if the direction of w and wOld is the same, this is why we test the two cases.
