...
//
// Michael Kleder has published a Java call to compute the MD5 and SHA sums: http://www.mathworks.com/matlabcentral/fileexchange/8944
/*******************************************************************************
...
*/
//Acquired by Eli Bowen 3/26/2019 from https://www.mathworks.com/matlabcentral/fileexchange/25921-getmd5
...
void ProcessArray (const mxArray *V, uchar_T digest[16])
{
    // The type, dimension and contents of the array are considered. This works for cells and structs also.
    // Here only the initialization and finalization of the context is performed,
    // while the actual processing is done in ArrayCore, which allows recursion for cells and structs.
    MD5_CTX context;
...
void ArrayCore (MD5_CTX *context, const mxArray *V)
{
    ...
    // They are forwarded to the helper function, where SIZE() replies the true dimensions.
    if (!dataOpaq) {
        // Consider class as name, not as ClassID, because the later might change with the Matlab version:
    ...
    switch (ClassID) {
        ...
        default:  // FUNCTION, VOID, OPAQUE, UNKNOWN CLASS: -----------------------
            ...
            if (mxIsUint8(Arg[0])) {     // Byte stream:
                MD5_Update(context, (uchar_T *) mxGetData(Arg[0]), mxGetNumberOfElements(Arg[0]));
...
void StructCore (MD5_CTX *context, const mxArray *V, mwSize nElem)
{
    ...
    // Sort the strings (Sorting must not be stable, because the fieldnames are unique):
...
{
    // Only the contents of the array is considered. Therefore double(0) and single([0,0]) reply the same hash. 
    // This works for numeric, char and logical arrays only, neither cells nor structs.
    MD5_CTX context;
...
void ToBase64 (const uchar_T In[16], char *Out)
{
    ...
    static const uchar_T B64[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    switch (Method) {
        ...
        case ASCII_m:   // Consider ASCII part of 16-bit mxChar only:
            ProcessChar((mxChar *) mxGetData(prhs[0]), mxGetNumberOfElements(prhs[0]), digest);
            break;
        ...
        case BINARY_m:  // Contents of the variable:
            ...
            || mxIsComplex(prhs[0]) || mxIsSparse(prhs[0])) {
                ERROR("BadDataType", "Binary mode requires: non-sparse, real, numeric or CHAR data.");
            }
