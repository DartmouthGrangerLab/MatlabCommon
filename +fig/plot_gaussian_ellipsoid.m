% downloaded from https://www.mathworks.com/matlabcentral/fileexchange/16543-plot-gaussian-ellipsoid by Eli Bowen 1/16/2018
...
%  plotted in the current axes. H is the graphics handle to the plotted ellipse or ellipsoid.
%
...
%  view(129,36); set(gca,'proj','perspective'); grid on; axis equal; axis tight;
% -------------------------------------------
...
function h = plot_gaussian_ellipsoid(m, C, sdwidth, npts, axh)
    ...
    assert(numel(m) == length(m), 'M must be a vector'); 
    assert(all(numel(m) == size(C)), 'Dimensionality of M and C must match');
