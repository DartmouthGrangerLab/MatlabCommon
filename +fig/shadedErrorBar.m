...
%   y - vector of y values or a matrix of n observations by m cases where m has length(x);
...
%            statistic the line should be and the second defines the error bar.
%   lineProps - [optional,'-k' by default] defines the properties of the data line
%               e.g. 'or-', or {'-or','markerfacecolor',[1,0.2,0.2]}
%   transparent - [optional, 0 by default] if ==1 the shaded error
%                 bar is made transparent, which forces the renderer to be openGl
%                 However, if this is saved as .eps the resulting file will contain a raster not a vector image. 
% RETURNS
...
function varargout = shadedErrorBar(x, y, err, lineProps, transparent)
    ...
    validateattributes(err, {'numeric','logical'}, {'nonempty'}, 3);
    x   = double(x);
    y   = double(y);
    err = double(err);
