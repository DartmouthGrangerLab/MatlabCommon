function [cepstra,aspectrum,pspectrum] = melfcc(samples, sr, varargin)
    ...
    [wintime, hoptime, numcep, lifterexp, sumpower, preemph, dither, ...
        minfreq, maxfreq, nbands, bwidth, dcttype, ...
        fbtype, usecmp, modelorder, broaden, useenergy] = ...
        process_options(varargin, 'wintime', 0.025, 'hoptime', 0.010, 'numcep', 13, 'lifterexp', 0.6, 'sumpower', 1, 'preemph', 0.97, 'dither', 0, ...
        'minfreq', 0, 'maxfreq', 4000, 'nbands', 40, 'bwidth', 1.0, 'dcttype', 2, ...
        'fbtype', 'mel', 'usecmp', 0, 'modelorder', 0, 'broaden', 0, 'useenergy', 0);
    ...
    if usecmp
        aspectrum = postaud(aspectrum, maxfreq, fbtype, broaden); %PLP-like weighting/compression
    end
