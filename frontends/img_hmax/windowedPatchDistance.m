...
%       c1Img and patch must have the same depth, as each layer represents one orientation
...
%       of pruning to conduct across each orientation of the patch. If 0, no pruning is done.
%
% RETURNS:
...
%       Each entry marks the distance between the patch and the image's C1 activations centered at that location.
%
% note: sumOverP(W(p)-I(p))^2 is computed as
%       sumOverP(W(p)^2) - 2*(W(p)*I(p)) + sumOverP(I(p)^2);
% modified by Eli Bowen 7/2021 for readability
function [distances] = windowedPatchDistance (c1Img, patch, ALLS2C1PRUNE, ORIENTATIONS2C1PRUNE)
    ...
    patchSz = size(patch);
    sumSupport = [ceil(patchSz(2)/2)-1,ceil(patchSz(1)/2)-1,...
                  floor(patchSz(2)/2),floor(patchSz(1)/2)];
    c1ImgSq = c1Img .^ 2;
    c1ImgSq = sumFilter(sum(c1ImgSq, 3), sumSupport);

    patchXc1Img = zeros(size(c1ImgSq));
    for i = 1:c1ImgDepth
        tmpPatch = flipud(fliplr(double(patch(:,:,i)))); % flipped for conv2
        patchXc1Img = patchXc1Img + conv2(c1Img(:,:,i), keepP(:,:,i) .* tmpPatch, 'same');
    end

    distSq = c1ImgSq - 2*patchXc1Img + sum(keepP(:) .* patch(:).^2);
    distSq(distSq < 0) = 0;
    distances = sqrt(distSq) + 10^-10;
end