function [fcoefs,cf] = MakeERBFilters (fs, numChannels, lowFreq)
    % function [fcoefs,cf]=MakeERBFilters(fs,numChannels,lowFreq)
    % This function computes the filter coefficients for a bank of Gammatone filters.
    ...
    % Each row of the filter arrays contains the coefficients for four second order filters.
    % The transfer function for these four filters share the same
    % denominator (poles) but have different numerators (zeros).
    % All of these coefficients are assembled into one vector that the ERBFilterBank
    ...
    % Alternatively, if the numChannels input argument is a vector, then the values of this vector are taken to
    % be the center frequency of each desired filter. (The lowFreq argument is ignored in this case.)
    %NOTE:
    % This implementation fixes a problem in the original code by
    ...
    % Thanks to Julius Smith for leading me to the proper explanation.
    %EXAMPLE:
    ...
    % Rewritten by Malcolm Slaney@Interval.  June 11, 1998. (c) 1998 Interval Research Corporation
