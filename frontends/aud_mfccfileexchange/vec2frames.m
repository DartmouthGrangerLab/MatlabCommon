...
%           D specifies if the frames in B are rows or columns, i.e. D = 'rows' or 'cols', respectively
...
%           P specifies if last frame should be padded to full length, or simply discarded, i.e., P = true or false, respectively
%           B is the output matrix of frames
%           R is a matrix of indexes used for framing, such that division of A into frames is achieved as follows: B=A(R);
%
%   Examples
    ...
    if ~isempty(padding)
        ...
        if E > 0
            ...
            if islogical(padding) && padding
                vec = [ vec; zeros(P,1) ]; %pad with zeros
            elseif isnumeric(padding) && length(padding)==1
                vec = [ vec; padding*ones(P,1) ]; %pad with a specific numeric constant
            elseif isstr(padding) && strcmp(padding, 'noise')
                vec = [ vec; 1E-6*randn(P,1) ]; %pad with a low variance white Gaussian noise
            elseif iscell(padding) && strcmp(padding{1}, 'noise')
                ...
                M = M-1; %if not padding required, decrement frame count (not a very elegant solution)
            end
