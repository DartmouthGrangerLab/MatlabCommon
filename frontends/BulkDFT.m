...
function [y] = BulkDFT (y, stepSz)
    ...
    nWindows = floor(numel(y) / stepSz);

    re = zeros(stepSz/2 + 1, nWindows, 'like', y);
    im = zeros(stepSz/2 + 1, nWindows, 'like', y);
    if isempty(gcp('nocreate')) % no parallel pool
        ...
        yChunks = zeros(stepSz, nWindows, 'like', y);
        for j = 1:nWindows
...
function [cosLookup,sinLookup] = BuildCosSinLookupTable (stepSz)
    validateattributes(stepSz, {'double','single'}, {'nonempty','scalar','positive','integer'});
