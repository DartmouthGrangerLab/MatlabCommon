% IIR LTV Q SPECTROGRAM Multiresolution constant quality spectrogram calculated with a varying IIR filtering of the DFT.
%
% INPUTS:
%   y: Audio Signal
...
%   f: Frequency vector. Contains the frequency values corresponding to each row of S.
%   t: Time vector. Containns the time values at which each column of S was calculated
%
% Example:
%   [s, t, f] = iir_ltv_q_spectrogram(y,4096,256,44100,13)
%downloaded by Eli Bowen and just cleaned up for appearance and consistency with other code
function [S,f,t] = iir_cqt_spectrogram (y, nfft, hop, fs, Q)
    y = y(:); %transform input data to a column vector
    ny = length(y);
    
    nframes = ceil(ny/hop); %number of frames (temporal index)
    frameindex = 1 + (0 : nframes - 1) * hop; %samples
    
    y = [y; zeros(nfft-mod(ny,hop),1)]; %zero padding at the end to complete the last frame
    
    S = zeros(nframes,nfft/2); %matrix to store the q-spectrum
    ...
    poles = calculate_poles(thetas,Q,nfft); %poles of the IIR LTV Q FFT transform for the parameters above
