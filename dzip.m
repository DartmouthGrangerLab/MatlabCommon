...
% Z = compressed output
%
% NOTES: (1) The input variable M can be a scalar, vector, matrix, or n-dimensional matrix
...
%            char, int8, uint8, int16, uint16, int32, uint32, int64, and uint64.
...
function Z = dzip (M)
    s = size(M);
    c = class(M);
    cn = strmatch(c,{'double','single','logical','char','int8','uint8','int16','uint16','int32','uint32','int64','uint64'});
    if cn == 3 || cn == 4
        M = uint8(M);
    end
    M = typecast(M(:),'uint8');
