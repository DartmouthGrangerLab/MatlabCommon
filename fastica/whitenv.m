...
% Whitens the data (row vectors) and reduces dimension.
...
function [newVectors, whiteningMatrix, dewhiteningMatrix] = whitenv (vectors, E, D, s_verbose)
    if nargin < 4, s_verbose = 'on'; end %default value for 'verbose'
    ...
    % In some cases, rounding errors in Matlab cause negative eigenvalues (elements in the diagonal of D).
    % Since it is difficult to know when this happens, it is difficult to correct it automatically. 
    % Therefore an error is signalled and the correction is left to the user.
    if any(diag(D) < 0)
        error(sprintf(['[ %d ] negative eigenvalues computed from the covariance matrix.\n'...
    ...
    % ========================================================
    if ~isreal(newVectors) %safety check
