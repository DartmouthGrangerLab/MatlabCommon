...
function [word,vec] = LoadWordVecs(name, path)
    ...
    if strcmp(name, 'wordvec_glove6')
        text = io.UnzipText(fullfile(path, 'glove', 'glove.6B.300d.txt.zip'));
    elseif strcmp(name, 'wordvec_glove42')
        text = io.UnzipText(fullfile(path, 'glove', 'glove.42B.300d.txt.zip'));
    elseif strcmp(name, 'wordvec_glove840')
        text = io.UnzipText(fullfile(path, 'glove', 'glove.840B.300d.txt.zip')); % 11 GB loaded
    elseif strcmp(name, 'wordvec_smalldensebin_glove6')
        text = io.UnzipText(fullfile(path, 'small_dense_binary', 'glove.6B.300d_with_header_binarized_trulybinary.vec.zip'));
    ...
    elseif strcmp(name, 'wordvec_largesparsebin_glove6x10')
        text = io.UnzipText(fullfile(path, 'large_sparse_binary', 'glove.6B.300d_sparsifiedx10.txt.zip')); % 6 GB loaded; 10% nonzero
    elseif strcmp(name, 'wordvec_largesparsebin_glove6x20')
        text = io.UnzipText(fullfile(path, 'large_sparse_binary', 'glove.6B.300d_sparsifiedx20.txt.zip'));
        error('untested');
    elseif strcmp(name, 'wordvec_largesparsebin_glove840x20')
        text = io.UnzipText(fullfile(path, 'large_sparse_binary', 'glove.840B.300d_sparsifiedx20.txt.zip'));
        error('untested');
    else
