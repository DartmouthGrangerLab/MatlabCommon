...
%INPUTS:
%   stringLabels - a 1D cell array of strings
%RETURNS:
%   numericIDs - a 1D numeric array the same length as stringLabels, where numericIDs(i) corresponds to stringLabels{i}
...
function [numericIDs,dictionary] = StringLabels2Numeric (stringLabels)
    ...
    for i = 1:numel(stringLabels)
        numericIDs(i) = StringFind(dictionary, stringLabels{i}, 1);
    end
end