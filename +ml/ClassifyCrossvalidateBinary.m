...
function [acc,accStdErr,predLabel,score,uniqueLabelOut] = ClassifyCrossvalidateBinary(data, label, n_folds, classifierType, classifierParams, verbose)
    ...
    for i = 1 : n_classes
        for j = i + 1 : n_classes
            ...
            if nargout() > 3
                [acc(i,j),accStdErr(i,j),predLabel{i,j},score{i,j}] = ml.ClassifyCrossvalidate(data(keep,:), (labelIdx(keep) == i) + 1, n_folds, classifierType, false, classifierParams, false);
            elseif nargout() > 2 % faster
                [acc(i,j),accStdErr(i,j),predLabel{i,j}]            = ml.ClassifyCrossvalidate(data(keep,:), (labelIdx(keep) == i) + 1, n_folds, classifierType, false, classifierParams, false);
            else % fasterer
                [acc(i,j),accStdErr(i,j)]                           = ml.ClassifyCrossvalidate(data(keep,:), (labelIdx(keep) == i) + 1, n_folds, classifierType, false, classifierParams, false);
            end
        end
    end
