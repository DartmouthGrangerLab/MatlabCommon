%  Determine best times of syllable onsets and ends in a speech-acoustic signal in silence.
% 	[vsegs,vlms,lms,tvals,pcont,env,envv,vf,tvv] = vowel_segs_std(SIGNAL,<Fs>,<MAX_F0>,...
% 																	<FIGNO>,<AGESZ>,<LMS>)
...
% 	<MAX_F0> = highest F0 or [max.,min.] to be considered when determining voicing; the 
% 			default value is the default of 'landmarks' and of 'vowel_lms' independently; 
% 			note that these two defaults may not be equal;
...
%    		etc.), for some N >= 0; each LM is denoted by a time & type (index), either
% 			copied from LMS (if supplied) or else filtered for the speech regions (per 
% 			'lm_sylfilter'); the indices are as in 'lm_labels' and 'lm_codes'; see Notes;
...
%  12.The function 'vowel_segs_full' provides similar functionality to this one, but 'lms'
% 	includes virtually all detected LMs, with only minimal filtering by 'lm_sylfilter'.
% 
%  See also: landmarks, vowel_lms, lm_labels, deg_speechenv, mat_vowel_segs, vowel_segs_nsil, 
% 		deg_voiced, vseg_ndxs_std, lm_sylfilter, vowel_segs_full, hpfilt_std.
%
