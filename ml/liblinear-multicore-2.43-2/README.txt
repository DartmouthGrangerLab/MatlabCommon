...
linear classification, regression and outlier detection. This
extension of LIBLINEAR supports multi-core parallel learning by
OpenMP. We applied efficient implementations to train models for
multi-core machines. Single-core LIBLINEAR currently supports
...
Some have been extended or modified to use multi cores. See ``Differences from Single-core LIBLINEAR'' for details.
...
- Differences from Single-core LIBLINEAR
...
- Python interface
- References for Multi-core Implementations
- Additional Information

Differences from Single-core LIBLINEAR
======================================

The following solvers can be run in parallel.

        Primal L2-regularized solvers (-s 0, -s 2)
        ------------------------------------------
        It does matrix-vector multiplications in parallel.

        Dual L2-regularized solvers (-s 1, -s 3, -s 11)
        -----------------------------------------------
        It calculates the gradients in parallel, and then conducts CD
        (coordinate descent) updates serially.

        Primal l1-regularized solvers (-s 5, -s 6)
        ------------------------------------------
        It parallelizes feature-wise operations for the features
        that have enough non-zeros.

Details of the above solvers can be found in the references.

Note: Parallelization on cross-validation or parameter search is turned off
by default. To enable it please refer to the section
"Parallelization on Cross-validation and Parameter Search (for ALL solvers)"
at https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/multicore-liblinear/
...
-m nr_thread: use nr_thread threads for parallelizing solvers
...
> train -s 0 -m 8 heart_scale

Run the L2-regularized logistic regression primal solver with 8 threads.

> train -s 3 -m 8 heart_scale

Run the L2-regularized l1-loss dual solver with 8 threads.
