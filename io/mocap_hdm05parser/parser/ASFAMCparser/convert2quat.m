% modified by Eli for only for readability
function mot = convert2quat(skel,mot)
    ...
    for k = 1:nTraj
        ...
        if nRotDOF > 0
            ...
            count = 1; % index for DOFs present in mot.rotationEuler
            for r = 1:3 % go through rotationOrder
                idx = strmatch(['r',lower(node.rotationOrder(r))], lower(node.DOF), 'exact');
                if ~isempty(idx)
                    completeEulers(r,:) = mot.rotationEuler{node.ID,1}(count,:);
                    count = count + 1;
                end
            end
            completeEulers = completeEulers .* conversion_factor;
