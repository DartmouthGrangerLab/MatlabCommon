...
%Modified only for readibility and to add this documentation
    ...
    for iTest = 1:size(TestData, 1)
        ...
        if strcmpi(Str, TestData{iTest, 2}) == 0
            error([ErrID, ':KAT'], ['Failed for char vector:', char(10), '[', TestData{iTest, 1}, ']']);
        end
        ...
        if strcmpi(Str3, TestData{iTest, 2}) == 0
            error([ErrID, ':KAT'], ['Failed for UINT8:', char(10), '[', TestData{iTest, 1}, ']']);
        end
    end
    disp('  ok: Test files');
    delete(TestFile);
    ...
    fprintf(['  ok: %d random tests with hex, HEX, double, uint8, base64 ', 'output: \n'], N);
