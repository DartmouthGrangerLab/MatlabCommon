...
%                       Then the digest is the same as for a ASCII text
...
%             'Array':  Include the class and dimensions of Data in the MD5 sum.
%                       This can be applied for (nested) structs, cells and sparse arrays also.
...
%
% COMPILATION:
%   The C-Mex-file is compiled automatically, when this function is called the first time.
...
% For more checksum methods see: http://www.mathworks.com/matlabcentral/fileexchange/31272-datahash
...
%Modified only for readibility and to add this documentation
    ...
    if isempty(FirstRun)
        ...
        try
            ...
            error(['JSimon:', mfilename, ':NoHelper'], '### %s: GetMD5_Helper.m: %s', mfilename, ME.message);
        end
    else
        error(['JSimon:', mfilename, ':MissMEX'], 'Cannot find Mex file: %s', [mfilename, '.', mexext]);
    end
end
